...
import net.ooder.skillcenter.service.k8s.K8sService;
...
public class K8sController {
    ...
    public ResponseEntity<Map<String, Object>> getStatus() {
        ...
        List<ClusterInfo> clusters = k8sService.listClusters();
        int totalNodes = 0;
        int totalPods = 0;
        int totalNamespaces = 0;
        
        for (ClusterInfo cluster : clusters) {
            totalNodes += cluster.getNodeCount() != null ? cluster.getNodeCount() : 0;
            totalPods += cluster.getPodCount() != null ? cluster.getPodCount() : 0;
            totalNamespaces += cluster.getNamespaceCount() != null ? cluster.getNamespaceCount() : 0;
        }
        
        status.put("clusterCount", clusters.size());
        status.put("nodeCount", totalNodes);
        status.put("podCount", totalPods);
        status.put("namespaceCount", totalNamespaces);
