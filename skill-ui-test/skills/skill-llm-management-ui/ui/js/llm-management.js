const LLMManagement = {
    ...
    async showModelDetail(providerId, modelId) {
        try {
            const response = await this.promisify(NexusAPI, 'get', `/api/llm/model/${providerId}/${modelId}`);
            ...
            if (modalTitle) modalTitle.textContent = model.displayName || modelId;
            
            if (modalBody) {
                modalBody.innerHTML = `
                    <div style="margin-bottom: 24px;">
                        <h4 style="margin: 0 0 8px 0; font-size: 20px;">${model.displayName || modelId}</h4>
                        <p style="margin: 0; color: var(--nx-text-secondary);">${model.description || ''}</p>
                    </div>
                    
                    <div class="nx-form-divider"><span>模型规格</span></div>
                    
                    <div class="nx-form-row">
                        <div class="nx-form-group">
                            <label class="nx-label">上下文窗口</label>
                            <div style="padding: var(--nx-space-2) 0; font-size: var(--nx-text-lg); font-weight: 600;">
                                ${model.contextWindow ? model.contextWindow.toLocaleString() + ' tokens' : '未知'}
                            </div>
                        </div>
                        <div class="nx-form-group">
                            <label class="nx-label">最大输出</label>
                            <div style="padding: var(--nx-space-2) 0; font-size: var(--nx-text-lg); font-weight: 600;">
                                ${model.maxOutputTokens ? model.maxOutputTokens.toLocaleString() + ' tokens' : '未知'}
                            </div>
                        </div>
                    </div>
                    
                    <div class="nx-form-divider"><span>能力支持</span></div>
                    
                    <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 12px;">
                        <div style="display: flex; align-items: center; gap: 8px; padding: var(--nx-space-3); background: var(--nx-bg-elevated); border-radius: var(--nx-radius);">
                            <i class="${model.supportsStreaming ? 'ri-check-circle-fill' : 'ri-close-circle-fill'}" 
                               style="font-size: 20px; color: ${model.supportsStreaming ? 'var(--ns-success)' : 'var(--ns-danger)'}"></i>
                            <span>流式输出</span>
                        </div>
                        <div style="display: flex; align-items: center; gap: 8px; padding: var(--nx-space-3); background: var(--nx-bg-elevated); border-radius: var(--nx-radius);">
                            <i class="${model.supportsFunctionCall ? 'ri-check-circle-fill' : 'ri-close-circle-fill'}" 
                               style="font-size: 20px; color: ${model.supportsFunctionCall ? 'var(--ns-success)' : 'var(--ns-danger)'}"></i>
                            <span>函数调用</span>
                        </div>
                        <div style="display: flex; align-items: center; gap: 8px; padding: var(--nx-space-3); background: var(--nx-bg-elevated); border-radius: var(--nx-radius);">
                            <i class="${model.supportsVision ? 'ri-check-circle-fill' : 'ri-close-circle-fill'}" 
                               style="font-size: 20px; color: ${model.supportsVision ? 'var(--ns-success)' : 'var(--ns-danger)'}"></i>
                            <span>视觉理解</span>
                        </div>
                        <div style="display: flex; align-items: center; gap: 8px; padding: var(--nx-space-3); background: var(--nx-bg-elevated); border-radius: var(--nx-radius);">
                            <i class="${model.supportsRAG ? 'ri-check-circle-fill' : 'ri-close-circle-fill'}" 
                               style="font-size: 20px; color: ${model.supportsRAG ? 'var(--ns-success)' : 'var(--ns-danger)'}"></i>
                            <span>RAG检索增强</span>
                        </div>
                    </div>
                    
                    <div class="nx-form-divider"><span>价格信息 (每1K tokens)</span></div>
                    
                    <div class="nx-form-row">
                        <div class="nx-form-group">
                            <label class="nx-label">输入价格</label>
                            <div style="padding: var(--nx-space-2) 0; font-size: var(--nx-text-lg); font-weight: 600; color: var(--ns-primary);">
                                $${model.inputPrice ? model.inputPrice.toFixed(4) : '0.0000'}
                            </div>
                        </div>
                        <div class="nx-form-group">
                            <label class="nx-label">输出价格</label>
                            <div style="padding: var(--nx-space-2) 0; font-size: var(--nx-text-lg); font-weight: 600; color: var(--ns-primary);">
                                $${model.outputPrice ? model.outputPrice.toFixed(4) : '0.0000'}
                            </div>
                        </div>
                    </div>
                    
                    <div class="nx-form-divider"><span>支持能力</span></div>
                    
                    <div style="display: flex; flex-wrap: wrap; gap: 8px;">
                        ${(model.capabilities || []).map(cap => `
                            <span style="padding: var(--nx-space-1) var(--nx-space-3); background: rgba(59, 130, 246, 0.1); color: var(--ns-primary); border-radius: var(--nx-radius-full); font-size: var(--nx-text-sm);">
                                ${cap}
                            </span>
                        `).join('')}
                    </div>
