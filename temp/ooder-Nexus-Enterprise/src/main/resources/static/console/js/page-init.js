...
(function() {
  ...
  const PageInit = {
    ...
    async _initMenu() {
      ...
      console.log('[PageInit] initMenu 是否存在:', typeof initMenu === 'function');
      console.log('[PageInit] MenuLoader 是否存在:', typeof MenuLoader !== 'undefined');
      
      // 使用统一的菜单初始化函数
      if (typeof initMenu === 'function') {
        console.log('[PageInit] 调用 initMenu()...');
        await initMenu();
        console.log('[PageInit] initMenu() 完成');
      } else if (typeof MenuLoader !== 'undefined') {
        console.log('[PageInit] 使用 MenuLoader...');
        const menuLoader = new MenuLoader();
        await menuLoader.init();
        console.log('[PageInit] MenuLoader.init() 完成');
      } else {
        console.warn('[PageInit] 菜单系统未加载');
      }
    },
  ...
  if (currentScript && currentScript.hasAttribute('data-auto-init')) {
    ...
    // 等待菜单系统加载
    const waitForMenu = (callback, maxWait = 1000) => {
      if (typeof initMenu === 'function' || typeof MenuLoader !== 'undefined') {
        callback();
      } else if (maxWait > 0) {
        setTimeout(() => waitForMenu(callback, maxWait - 50), 50);
      } else {
        console.warn('[PageInit] 菜单系统加载超时');
        callback();
      }
    };
    ...
    // 检查是否所有依赖都已加载
    if (document.readyState === 'loading') {
      document.addEventListener('DOMContentLoaded', () => waitForMenu(doInit));
    } else {
      waitForMenu(doInit);
    }
  }
