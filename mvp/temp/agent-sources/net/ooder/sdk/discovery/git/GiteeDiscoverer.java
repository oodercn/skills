...
/**
 * Gitee Repository Discoverer Implementation
 * 
 * Supports two modes:
 ...
 * @since 2.0
 */
public class GiteeDiscoverer implements GitRepositoryDiscoverer {

    private static final Logger logger = LoggerFactory.getLogger(GiteeDiscoverer.class);
    ...
    public GiteeDiscoverer() {
        this(GitDiscoveryConfig.forGitee());
    }

    public GiteeDiscoverer(String token) {
        this(GitDiscoveryConfig.forGitee(token));
    }

    public GiteeDiscoverer(String token, String defaultOwner) {
        this(GitDiscoveryConfig.forGitee(token, defaultOwner));
    }

    public GiteeDiscoverer(String token, String defaultOwner, String skillsRepo) {
        this(GitDiscoveryConfig.forGitee(token, defaultOwner, skillsRepo));
    }
        ...
        return CompletableFuture.supplyAsync(() -> {
            try {
                ...
                if (config.isSingleRepoMode()) {
                    dir.setHtmlUrl(config.getWebBaseUrl() + "/" + owner + "/" + config.getSkillsRepo() + "/tree/master/" + config.getSkillsPath() + "/" + skillName);
                } else {
    ...
    public CompletableFuture<List<SkillDirectory>> listSkillDirectories(String owner, String skillsPath) {
        ...
        return CompletableFuture.supplyAsync(() -> {
            ...
            try {
                String apiUrl;
                if (config.isSingleRepoMode()) {
                    apiUrl = config.getApiBaseUrl() + "/repos/" + owner + "/" + config.getSkillsRepo() + "/contents/" + encodePath(skillsPath);
                } else {
                    apiUrl = config.getApiBaseUrl() + "/repos/" + owner + "/contents/" + encodePath(skillsPath);
                }
                
                if (config.getToken() != null && !config.getToken().isEmpty()) {
                    apiUrl += "?access_token=" + config.getToken();
                }
    ...
    public CompletableFuture<List<ReleaseInfo>> getReleases(String owner, String repoName) {
        return CompletableFuture.supplyAsync(() -> {
            ...
            try {
                ...
                if (config.getToken() != null && !config.getToken().isEmpty()) {
                    releasesUrl += "?access_token=" + config.getToken();
                }
    ...
    public CompletableFuture<ReleaseInfo> getLatestRelease(String owner, String repoName) {
        return CompletableFuture.supplyAsync(() -> {
            try {
                ...
                if (config.getToken() != null && !config.getToken().isEmpty()) {
                    releaseUrl += "?access_token=" + config.getToken();
                }
    ...
    private String buildManifestUrl(String owner, String skillName) {
        String path;
        if (config.isSingleRepoMode()) {
            path = config.getSkillsPath() + "/" + skillName + "/" + MANIFEST_FILE;
        } else {
            path = MANIFEST_FILE;
        }
        
        String url = config.getApiBaseUrl() + "/repos/" + owner + "/" 
            + (config.isSingleRepoMode() ? config.getSkillsRepo() : skillName) 
            + "/contents/" + encodePath(path);
        
        if (config.getToken() != null && !config.getToken().isEmpty()) {
            url += "?access_token=" + config.getToken();
        }
        return url;
    }
    ...
    private JsonNode executeRequest(String urlString) {
        try {
            ...
            if (responseCode == 200) {
                ...
                logger.warn("Gitee API request failed: {} - {}", responseCode, urlString);
            }
        } catch (Exception e) {
    ...
    private ReleaseInfo parseReleaseInfo(JsonNode release) {
        ...
        if (release.has("assets") && release.get("assets").isArray()) {
            for (JsonNode asset : release.get("assets")) {
                ...
                assetInfo.setAssetId(asset.has("id") ? String.valueOf(asset.get("id").asInt()) : "");
