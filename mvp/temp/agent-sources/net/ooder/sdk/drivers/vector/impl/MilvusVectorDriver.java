...
@DriverImplementation(value = "VectorDriver", skillId = "skill-vector-milvus")
public class MilvusVectorDriver implements VectorDriver {
    
    private static final Logger log = LoggerFactory.getLogger(MilvusVectorDriver.class);
    ...
    public void init(VectorConfig config) {
        this.config = config;
        connected.set(true);
        log.info("Milvus vector store initialized (simulated mode)");
    }
    ...
    public void close() {
        ...
        collections.clear();
        connected.set(false);
        log.info("Milvus vector store closed");
    }
