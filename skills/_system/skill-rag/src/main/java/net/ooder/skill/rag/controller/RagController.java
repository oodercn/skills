...
public class RagController {
    ...
    @PostMapping("/ingest")
    public ResultModel<RagPipeline.KnowledgeDocument> ingestBusinessData(
        ...
        try {
            RagPipeline.KnowledgeDocument doc = ragPipeline.ingestBusinessData(request);
    ...
    @GetMapping("/search")
    public ResultModel<List<RagPipeline.KnowledgeDocument>> searchRelated(
            @RequestParam String query,
            @RequestParam(defaultValue = "5") int limit) {
        log.info("[RagController] searchRelated: query={}, limit={}", query, limit);
        try {
            List<RagPipeline.KnowledgeDocument> docs = ragPipeline.searchRelated(query, limit);
