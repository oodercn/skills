(function() {
    ...
    function safeJsonParse(data, defaultValue) {
        ...
        try {
            ...
            console.warn('[install-scene] JSON解析失败:', error.message, '原始数据:', data);
            return defaultValue || null;
        }
    }
    ...
    function startInstall() {
        ...
        eventSource.addEventListener('skill', function(e) {
            ...
            if (data && data.skillId) {
                ...
                addLog('正在处理: ' + (data.name || data.skillId) + ' (' + (data.index || '?') + '/' + (data.total || '?') + ')', 'info');
            }
        });
        ...
        eventSource.addEventListener('skillProgress', function(e) {
            ...
            if (data && data.skillId) {
                if (data.status === 'installed') {
                    ...
                    addLog('✓ ' + (data.message || '安装成功'), 'success');
                } else if (data.status === 'skipped') {
                    updateSkillUI(data.skillId, 'skipped');
                    addLog('○ ' + (data.message || '已跳过'), 'info');
                }
            }
        });
