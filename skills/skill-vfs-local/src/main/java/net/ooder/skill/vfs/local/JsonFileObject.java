...
public class JsonFileObject implements FileObject, Cacheable, Serializable {
    ...
    public MD5InputStream downLoad() throws JDSException {
        ...
        FileAdapter fileAdapter = this.getFileAdapter();
    ...
    public Integer writeLine(String str) {
        Integer ln = -1;
        FileAdapter fileAdater = getFileAdapter();
        String vfsPath = getPath();
        if (vfsPath != null && fileAdater.exists(vfsPath)) {
            ln = fileAdater.writeLine(vfsPath, str);
            setLength(fileAdater.getLength(vfsPath));
            setHash(fileAdater.getMD5Hash(vfsPath));
        }
        return ln;
    }
    ...
    private FileAdapter getFileAdapter() {
        return LocalVfsConfig.getFileAdapter(rootPath);
    }
    ...
    public int getCachedSize() {
        ...
        size += CacheSizes.sizeOfObject(updateTime);
        return size;
    }
    ...
    public static JsonFileObject fromJson(String json) {
        return JSON.parseObject(json, JsonFileObject.class);
    }
}
