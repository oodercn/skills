...
public class SceneController extends BaseController {
    ...
    public ResultModel<PageResult<SceneSnapshotDTO>> listSnapshots(@RequestBody SceneIdPageRequest request) {
        long startTime = System.currentTimeMillis();
        logRequestStart("listSnapshots", request);

        try {
            PageResult<SceneSnapshotDTO> result = sceneService.listSnapshots(
                request.getSceneId(), request.getPageNum(), request.getPageSize());
            logRequestEnd("listSnapshots", result, System.currentTimeMillis() - startTime);
            return ResultModel.success(result);
        } catch (Exception e) {
    ...
    @PostMapping("/logs")
    public ResultModel<PageResult<LogDTO>> getLogs(@RequestBody LogRequest request) {
        long startTime = System.currentTimeMillis();
        logRequestStart("getLogs", request);

        try {
            PageResult<LogDTO> result = sceneService.getLogs(
                request.getSceneId(), request.getLevel(), 
                request.getStartTime(), request.getEndTime(),
                request.getPageNum(), request.getPageSize());
            logRequestEnd("getLogs", result, System.currentTimeMillis() - startTime);
            return ResultModel.success(result);
        } catch (Exception e) {
    ...
    public static class LogRequest extends SceneIdPageRequest {
