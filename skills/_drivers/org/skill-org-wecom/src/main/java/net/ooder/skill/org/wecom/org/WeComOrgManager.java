package net.ooder.skill.org.wecom.org;
...
@Component
public class WeComOrgManager implements OrgManager {

    private static final Logger logger = LoggerFactory.getLogger(WeComOrgManager.class);
    ...
    public List<Org> getTopOrgs(String sysId) {
        try {
            List<WeComDepartment> depts = wecomClient.getDepartments(null);
            List<Org> orgs = new ArrayList<>();
            for (WeComDepartment dept : depts) {
                if (dept.getParentId() == null || "1".equals(dept.getParentId()) || "0".equals(dept.getParentId())) {
    ...
    public Org getOrgByID(String orgId) throws OrgNotFoundException {
        ...
        try {
            WeComDepartment dept = wecomClient.getDepartment(orgId);
            if (dept == null) {
    ...
    public Person getPersonByID(String personId) throws PersonNotFoundException {
        ...
        try {
            WeComUser user = wecomClient.getUser(personId);
            if (user == null) {
    ...
    public Person getPersonByAccount(String account) throws PersonNotFoundException {
        try {
            WeComUser user = wecomClient.getUser(account);
            if (user == null) {
    ...
    public Person getPersonByMobile(String mobile) throws PersonNotFoundException {
        try {
            WeComUser user = wecomClient.getUserByMobile(mobile);
            if (user == null) {
    ...
    public Person getPersonByEmail(String email) throws PersonNotFoundException {
        throw new PersonNotFoundException("WeCom does not support getting user by email");
    }

    @Override
    public List<Person> getPersons() {
        try {
            List<WeComUser> users = wecomClient.getAllUsers();
    ...
    public List<Person> getPersonsByOrgID(String orgId) {
        try {
            List<WeComUser> users = wecomClient.getUsersByDepartment(orgId);
            List<Person> persons = new ArrayList<>();
            for (WeComUser user : users) {
                persons.add(convertToPerson(user));
            }
    ...
    public boolean verifyPerson(String account, String password) {
        try {
            return wecomClient.verifyUser(account, password);
        } catch (Exception e) {
    ...
    public void reloadAll() {
        orgCache.clear();
        personCache.clear();
        logger.info("WeCom org cache cleared");
    }
    ...
    public PersonPrivateGroup getPrivateGroupById(String personGroupId) throws PersonPrivateGroupNotFoundException, JDSException {
        throw new PersonPrivateGroupNotFoundException("Private group not supported in WeCom");
    }

    @Override
    public Role getOrgLevelByID(String orgLevelId) throws RoleNotFoundException {
        throw new RoleNotFoundException("Org level not supported in WeCom");
    }
    ...
    public Role getRoleByID(String roleId) throws RoleNotFoundException {
        throw new RoleNotFoundException("Role not supported in WeCom");
    }
    ...
    public List<Org> getOrgs() {
        try {
            ...
            logger.error("Failed to get orgs", e);
            return new ArrayList<>();
        }
    }
    ...
    public Role getPersonDutyByID(String personDutyId) throws RoleNotFoundException {
        throw new RoleNotFoundException("Person duty not supported in WeCom");
    }
    ...
    public Role getPersonGroupByID(String personGroupId) throws RoleNotFoundException {
        throw new RoleNotFoundException("Person group not supported in WeCom");
    }
    ...
    public Role getPersonLevelByID(String personLevelId) throws RoleNotFoundException {
        throw new RoleNotFoundException("Person level not supported in WeCom");
    }
    ...
    public Role getPersonPositionByID(String personPositionId) throws RoleNotFoundException {
        throw new RoleNotFoundException("Person position not supported in WeCom");
    }
    ...
    public Role getPersonRoleByID(String personRoleId) throws RoleNotFoundException {
        throw new RoleNotFoundException("Person role not supported in WeCom");
    }
    ...
    private Org convertToOrg(WeComDepartment dept) {
        WeComOrg org = new WeComOrg();
        org.setOrgId(dept.getDeptId());
        org.setName(dept.getName());
        org.setParentId(dept.getParentId());
        org.setIndex(dept.getOrder() != null ? dept.getOrder() : 0);
        return org;
    }

    private Person convertToPerson(WeComUser user) {
        WeComPerson person = new WeComPerson();
        ...
        person.setStatus(user.getStatus() != null ? String.valueOf(user.getStatus()) : "1");
        return person;
    }
}
