...
@Component
public class FeishuOrgManager implements OrgManager {

    private static final Logger logger = LoggerFactory.getLogger(FeishuOrgManager.class);

    @Autowired
    private FeishuApiClient feishuClient;
    ...
    public List<Org> getTopOrgs(String sysId) {
        try {
            List<FeishuDepartment> depts = feishuClient.getDepartments(null);
    ...
    public Org getOrgByID(String orgId) throws OrgNotFoundException {
        ...
        try {
            FeishuDepartment dept = feishuClient.getDepartment(orgId);
            if (dept == null) {
    ...
    public Person getPersonByID(String personId) throws PersonNotFoundException {
        ...
        try {
            FeishuUser user = feishuClient.getUser(personId);
            if (user == null) {
    ...
    public Person getPersonByAccount(String account) throws PersonNotFoundException {
        try {
            FeishuUser user = feishuClient.getUserByAccount(account);
            if (user == null) {
    ...
    public Person getPersonByMobile(String mobile) throws PersonNotFoundException {
        try {
            FeishuUser user = feishuClient.getUserByMobile(mobile);
            if (user == null) {
    ...
    public Person getPersonByEmail(String email) throws PersonNotFoundException {
        try {
            FeishuUser user = feishuClient.getUserByEmail(email);
            if (user == null) {
    ...
    public List<Person> getPersons() {
        try {
            List<FeishuUser> users = feishuClient.getAllUsers();
    ...
    public List<Person> getPersonsByOrgID(String orgId) {
        try {
            List<FeishuUser> users = feishuClient.getUsersByDepartment(orgId);
            List<Person> persons = new ArrayList<>();
            for (FeishuUser user : users) {
                persons.add(convertToPerson(user));
            }
    ...
    public boolean verifyPerson(String account, String password) {
        try {
            return feishuClient.verifyUser(account, password);
        } catch (Exception e) {
    ...
    public void reloadAll() {
        orgCache.clear();
        personCache.clear();
        logger.info("Feishu org cache cleared");
    }
    ...
    public PersonPrivateGroup getPrivateGroupById(String personGroupId) throws PersonPrivateGroupNotFoundException, JDSException {
        throw new PersonPrivateGroupNotFoundException("Private group not supported in Feishu");
    }

    @Override
    public Role getOrgLevelByID(String orgLevelId) throws RoleNotFoundException {
        throw new RoleNotFoundException("Org level not supported in Feishu");
    }
    ...
    public Role getRoleByID(String roleId) throws RoleNotFoundException {
        throw new RoleNotFoundException("Role not supported in Feishu");
    }
    ...
    public Role getPersonDutyByID(String personDutyId) throws RoleNotFoundException {
        throw new RoleNotFoundException("Person duty not supported in Feishu");
    }
    ...
    public Role getPersonGroupByID(String personGroupId) throws RoleNotFoundException {
        throw new RoleNotFoundException("Person group not supported in Feishu");
    }
    ...
    public Role getPersonLevelByID(String personLevelId) throws RoleNotFoundException {
        throw new RoleNotFoundException("Person level not supported in Feishu");
    }
    ...
    public Role getPersonPositionByID(String personPositionId) throws RoleNotFoundException {
        throw new RoleNotFoundException("Person position not supported in Feishu");
    }
    ...
    public Role getPersonRoleByID(String personRoleId) throws RoleNotFoundException {
        throw new RoleNotFoundException("Person role not supported in Feishu");
    }
    ...
    private Org convertToOrg(FeishuDepartment dept) {
        FeishuOrg org = new FeishuOrg();
        org.setOrgId(dept.getOpenDepartmentId());
        org.setName(dept.getName());
        org.setParentId(dept.getParentDepartmentId());
        return org;
    }

    private Person convertToPerson(FeishuUser user) {
        FeishuPerson person = new FeishuPerson();
        person.setID(user.getOpenId());
        person.setAccount(user.getUserId());
        person.setName(user.getName());
        person.setNickName(user.getNickname());
        person.setMobile(user.getMobile());
        person.setEmail(user.getEmail());
        person.setOrgId(user.getDepartmentId());
        return person;
    }
}
