...
public class VolcEngineLlmProvider implements LlmProvider {
    ...
    private final VolcEngineLlmDriver driver;
    private String defaultModel;

    public VolcEngineLlmProvider() {
        this.driver = new VolcEngineLlmDriver();
        LlmDriver.LlmConfig config = new LlmDriver.LlmConfig();
        config.setApiKey(System.getenv("VOLCENGINE_API_KEY"));
        driver.init(config);
        this.defaultModel = System.getenv("VOLCENGINE_ENDPOINT_ID");
    }

    public VolcEngineLlmProvider(String apiKey, String endpointId) {
        this.driver = new VolcEngineLlmDriver();
        LlmDriver.LlmConfig config = new LlmDriver.LlmConfig();
        config.setApiKey(apiKey);
        driver.init(config);
        this.defaultModel = endpointId;
    }
    ...
    public List<String> getSupportedModels() {
        try {
            ...
            return Arrays.asList("doubao-pro-4k", "doubao-pro-32k", "doubao-lite-4k");
        }
    }
