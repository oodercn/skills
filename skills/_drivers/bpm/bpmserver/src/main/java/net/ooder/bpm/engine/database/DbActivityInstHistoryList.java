/**
 * $RCSfile: DbActivityInstHistoryList.java,v $
 * $Revision: 1.1 $
 ...
 * @version 1.0
 */
public class DbActivityInstHistoryList extends DbWorkflowList implements
		Serializable {
	 ...
	 */
	public DbActivityInstHistoryList() {
		super();
	}
	 ...
	 * @param c
	 */
	public DbActivityInstHistoryList(Collection c) {
		super(c);
	}

	public DbActivityInstHistoryList(DbActivityInstHistory[] acts) {
		size = acts.length;
	...
	protected Object getWorkflowObject(Object obj) {
		DbActivityInstHistory activityInst = (DbActivityInstHistory) obj;
		try {
			EIActivityInstHistory result = EIActivityInstHistoryManager
					.getInstance().loadByKey(
							activityInst.getActivityHistoryId());
			return result;
	...
	private void prefetch() {
		...
		for (; prefetchIndex < length; prefetchIndex++) {
			...
			DbActivityInstHistory activityInst = (DbActivityInstHistory) elementData[prefetchIndex];
			String uuid = activityInst.getActivityHistoryId();
			v.add(uuid);
		}
		DbActivityInstHistoryManager instManager = (DbActivityInstHistoryManager) EIActivityInstHistoryManager
				.getInstance();
		instManager.prepareCache(v);
	}
