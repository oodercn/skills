/**
 * $RCSfile: DbProcessDefList.java,v $
 * $Revision: 1.1 $
 ...
 * @version 1.0
 */
public class DbProcessDefList extends DbWorkflowList implements Serializable {
	 ...
	 * @param c
	 */
	public DbProcessDefList(Collection c) {
		super(c);
	}

	public DbProcessDefList(DbProcessDef[] processDefs) {
		size = processDefs.length;
		// Allow 10% room for growth
		elementData = new Object[size];
		System.arraycopy(processDefs, 0, elementData, 0, size);
	}
	...
	protected Object getWorkflowObject(Object obj) {
		DbProcessDef processDef = (DbProcessDef) obj;
		try {
			EIProcessDef result = EIProcessDefManager.getInstance().loadByKey(
	...
	private void prefetch() {
		...
		for (; prefetchIndex < length; prefetchIndex++) {
			...
			DbProcessDef processDef = (DbProcessDef) elementData[prefetchIndex];
			String uuid = processDef.getProcessDefId();
			v.add(uuid);
		}
		DbProcessDefManager defManager = (DbProcessDefManager) EIProcessDefManager
				.getInstance();
