/**
 * $RCSfile: DbProcessInstList.java,v $
 * $Revision: 1.1 $
 ...
 * @version 1.0
 */
public class DbProcessInstList extends DbWorkflowList implements Serializable {
	 ...
	 * @param c
	 */
	public DbProcessInstList(Collection c) {
		super(c);
	}

	public DbProcessInstList(DbProcessInst[] acts) {
		size = acts.length;
	...
	protected Object getWorkflowObject(Object obj) {
		DbProcessInst processInst = (DbProcessInst) obj;
		try {
			EIProcessInst result = EIProcessInstManager.getInstance()
					.loadByKey(processInst.getProcessInstId());
			return result;
	...
	private void prefetch() {
		...
		for (; prefetchIndex < length; prefetchIndex++) {
			...
			DbProcessInst processInst = (DbProcessInst) elementData[prefetchIndex];
			String uuid = processInst.getProcessInstId();
			v.add(uuid);
		}
		DbProcessInstManager instManager = (DbProcessInstManager) EIProcessInstManager
				.getInstance();
