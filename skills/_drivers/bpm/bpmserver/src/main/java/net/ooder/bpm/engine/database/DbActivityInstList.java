/**
 * $RCSfile: DbActivityInstList.java,v $
 * $Revision: 1.1 $
 ...
 * @version 1.0
 */
public class DbActivityInstList extends DbWorkflowList implements Serializable {
	 ...
	 * @param c
	 */
	public DbActivityInstList(Collection c) {
		super(c);
	}

	public DbActivityInstList(DbActivityInst[] acts) {
		size = acts.length;
	...
	protected Object getWorkflowObject(Object obj) {
		DbActivityInst activityInst = (DbActivityInst) obj;
		try {
			EIActivityInst result = EIActivityInstManager.getInstance()
					.loadByKey(activityInst.getActivityInstId());
			return result;
	...
	private void prefetch() {
		...
		for (; prefetchIndex < length; prefetchIndex++) {
			...
			DbActivityInst activityInst = (DbActivityInst) elementData[prefetchIndex];
			String uuid = activityInst.getActivityInstId();
			v.add(uuid);
		}
		DbActivityInstManager instManager = (DbActivityInstManager) EIActivityInstManager
				.getInstance();
