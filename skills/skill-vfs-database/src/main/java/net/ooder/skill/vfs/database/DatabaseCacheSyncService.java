...
public class DatabaseCacheSyncService {

    private static final Log log = LogFactory.getLog("vfs", DatabaseCacheSyncService.class);

    private static final int DEFAULT_PORT = 9877;
    private static final String MULTICAST_GROUP = "239.255.255.251";

    private static DatabaseCacheSyncService instance;
    ...
    private DBFileObjectManager fileObjectManager;

    private DatabaseCacheSyncService() {
        this.port = getPortFromConfig();
        this.multicastGroup = getMulticastGroupFromConfig();
    }

    public static synchronized DatabaseCacheSyncService getInstance() {
        if (instance == null) {
            instance = new DatabaseCacheSyncService();
        }
        return instance;
    }

    public void setFileObjectManager(DBFileObjectManager manager) {
    ...
    public void start() {
        ...
        try {
            ...
            log.info("DatabaseCacheSyncService started on port " + port);
        } catch (SocketException e) {
            log.error("Failed to start DatabaseCacheSyncService", e);
        }
    }
    ...
    public void stop() {
        ...
        log.info("DatabaseCacheSyncService stopped");
    }
    ...
    public void broadcastCacheInvalidation(String action, String fileId) {
        ...
        try {
            ...
            message.put("type", "vfs-db-cache-sync");
    ...
    private void processMessage(String message) {
        try {
            ...
            if (!"vfs-db-cache-sync".equals(type)) {
                return;
            }
    ...
    private int getPortFromConfig() {
        String portStr = System.getProperty("vfs.db.sync.port", System.getenv("VFS_DB_SYNC_PORT"));
        if (portStr != null) {
            try {
                return Integer.parseInt(portStr);
            } catch (NumberFormatException e) {
                log.warn("Invalid VFS DB sync port: " + portStr + ", using default");
            }
        }
        return DEFAULT_PORT;
    }
    ...
    private String getMulticastGroupFromConfig() {
        String group = System.getProperty("vfs.db.sync.multicast", System.getenv("VFS_DB_SYNC_MULTICAST"));
