package net.ooder.skill.notify.provider;
...
public class DefaultNotifyProvider implements NotifyProvider {
    ...
    public List<String> getSupportedChannels() {
        return Arrays.asList("web", "sms", "email", "push", "websocket");
    }
    
    @Override
    public NotifyResult send(NotifyRequest request) {
        log.info("Send notification: channel={}, recipients={}", request.getChannel(), request.getRecipients());
    ...
    public NotifyResult sendBatch(List<NotifyRequest> requests) {
        log.info("Send batch notifications: count={}", requests.size());
        ...
        for (NotifyRequest req : requests) {
            result.getSentTo().addAll(req.getRecipients());
        }
    ...
    public NotifyResult sendTemplate(String templateId, Map<String, Object> params, List<String> recipients) {
        log.info("Send template notification: templateId={}, recipients={}", templateId, recipients);
    ...
    public TemplateResult createTemplate(TemplateRequest request) {
        log.info("Create template: templateId={}", request.getTemplateId());
    ...
    public SubscriptionResult subscribe(String userId, String channel, Map<String, Object> config) {
        log.info("Subscribe: userId={}, channel={}", userId, channel);
