...
public class FeishuChannel implements MessageChannel {
    ...
    @Value("${channel.feishu.enabled:true}")
    private boolean enabled;
    
    @Value("${channel.feishu.endpoint:http://localhost:8083}")
    ...
    public PushResultDTO send(PushRequestDTO request) {
        log.info("[Feishu] Sending message to: {}", request.getReceiver());
        
        if (!enabled) {
            return PushResultDTO.fail("feishu", "飞书渠道未启用");
        }
        
        try {
            String url = endpoint + "/api/v1/im/feishu/send";
            ...
            if (response.getStatusCode().is2xxBody() && response.getBody() != null) {
                ...
                if ("success".equals(result.get("status")) || Integer.valueOf(200).equals(result.get("code"))) {
                    ...
                    return PushResultDTO.success(messageId, "feishu");
                }
            }
            
            return PushResultDTO.fail("feishu", "发送失败");
            
        } catch (Exception e) {
            log.error("Failed to send Feishu message", e);
            return PushResultDTO.fail("feishu", e.getMessage());
        }
    }
